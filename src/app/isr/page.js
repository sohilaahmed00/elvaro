...
export default async function ISRPage() {
  ...
  return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        <h1 style={titleStyle}>ISR</h1>
        {error && <div style={errorBadge}>⚠️ Offline Mode (Showing Fallback Data)</div>}
        <p style={descStyle}>
          This page is static but <strong>revalidates</strong> every 10 seconds.
          Refresh after 10 seconds to see the time update.
        </p>
        <div style={infoBox}>
          <h3>Data from API:</h3>
          {users.map(u => <h1 key={u.id}>{u.name}</h1>)}
          <hr style={hrStyle} />
          <p><strong>Rendered at:</strong> {time} (Updates every 10s)</p>
        </div>
      </div>
    </div>
  );
